function setCompassDirection(angle) {
	var currcanvas = document.getElementById("compasscanvas");
	currcanvas.width=currcanvas.width;
	var currcontext = currcanvas.getContext("2d");
	currcontext.beginPath();
	currcontext.arc(75, 75, 70, 0, Math.PI * 2, false);
	currcontext.closePath();
	currcontext.strokeStyle = "#000";
	currcontext.stroke();
	currcontext.font = "bold 15px verdana";
	currcontext.fillText("W", 10, 80);
	currcontext.fillText("E", 130, 80);
	currcontext.fillText("N", 70, 25);
	currcontext.fillText("S", 70, 140);
	currcontext.beginPath();
	currcontext.arc(75, 75, 5, 0, Math.PI * 2, false);
	currcontext.closePath();
	currcontext.fillStyle = "#000";
	currcontext.fill();
	angle*=Math.PI/180;
	angle-=Math.PI/2;
	x = 75+(40*Math.cos(angle));
	y = 75+(40*Math.sin(angle));
	currcontext.beginPath();
	currcontext.moveTo(75, 75);
...
function setAccelerometerGraph() {	
	var currcanvas = document.getElementById("accelerometercanvas");
	currcanvas.width=currcanvas.width;
	var currcontext = currcanvas.getContext("2d");
	currcontext.beginPath();
